...
import { UserCheck, Search, Filter, Calendar, Clock, CheckCircle, XCircle, Eye, X } from 'lucide-react';
...
export function AssistantAttendance() {
  ...
  useEffect(() => {
    api.get<{ attendanceSessions: AttendanceSession[]; studentAttendance: Student[] }>('/assistant/attendance')
      .then((data) => {
        setAttendanceSessions(data.attendanceSessions);
        setStudentAttendance(data.studentAttendance);
      })
      .catch((error) => console.error('Failed to load assistant attendance:', error));
  }, []);
